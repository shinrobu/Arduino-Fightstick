...
#define ANALOG_AXIS_ARRAY_SIZE 12
  ...
  {
    uint8_t YOn : 1;  // Each of these member variables control if a button is off or on. For the buttons, 0 is off 1 is on
    uint8_t BOn : 1; 
    uint8_t XOn : 1;    
    uint8_t AOn : 1; 
    uint8_t LBOn : 1;        
    uint8_t LTOn : 1;        
    uint8_t l3On : 1;        // The : 1 here just tells the compiler to only have 1 bit for each variable. This saves a lot of space for our type!
    uint8_t RBOn : 1;
    uint8_t RTOn : 1;
    uint8_t r3On : 1;
    uint8_t selectOn : 1;
...
//----- Below here is the actual implementation of
  ...
  megaJoyControllerData_t getBlankDataForMegaController(void){
    ...
    controllerData.YOn = 0;
    controllerData.BOn = 0;
    controllerData.XOn = 0;
    controllerData.AOn = 0;
    controllerData.LBOn = 0;
    controllerData.LTOn = 0;
    //controllerData.l3On = 0;
    controllerData.RBOn = 0;
    controllerData.RTOn = 0;
